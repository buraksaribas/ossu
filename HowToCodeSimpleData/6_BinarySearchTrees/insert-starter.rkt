...
;; adds a node that has the given key and value to the tree. The node should be inserted in the proper place in the tree.
(check-expect (insert-node 2 "qwe" BST4) (make-node 4 "dcj" (make-node 2 "qwe" false false) (make-node 7 "ruf" false false)))
(check-expect (insert-node 2 "qwe" BST1) (make-node 1 "abc" false (make-node 2 "qwe" false false)))

;(define (insert-node k v t) empty)
